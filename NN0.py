...
def main():
    """Run this experiment"""
    training_ints = initialize_instances('a_trg.csv')
    testing_ints = initialize_instances('a_test.csv')
    validation_ints = initialize_instances('a_val.csv')
    factory = BackPropagationNetworkFactory()
    measure = SumOfSquaresError()
    data_set = DataSet(training_ints)
    relu = RELU()
    logisticact = LogisticSigmoid()
    rule = RPROPUpdateRule()
    oa_names = ["Backprop"]
    classification_network = factory.createClassificationNetwork([INPUT_LAYER, HIDDEN_LAYER1,HIDDEN_LAYER2,HIDDEN_LAYER3, OUTPUT_LAYER],logisticact)
